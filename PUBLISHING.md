...
Run `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\Package.ps1` after a successful build. It creates fresh Source and Windows-x64 archives under `dist`, with SHA-256 files. Packaging uses an explicit file list and never reads the user's saved application settings. Existing archives with the same version are not overwritten; use a new output directory when repackaging. Keep the generated `VoiceMeeter-AEC-{version}-Windows-x64.zip` name and attach its `.sha256` file to the same stable GitHub release: the in-app updater requires that pair.
