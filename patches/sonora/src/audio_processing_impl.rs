...
impl AudioProcessingImpl {
    ...
    fn initialize_echo_controller(&mut self) {
        ...
        let mut stereo = EchoCanceller3Config::create_default_multichannel_config();
        stereo.echo_removal_control.transparent_mode = ec.transparent_mode;
        if ec.residual_suppression != crate::config::ResidualSuppression::Standard {
            let (transparent, suppress) = match ec.residual_suppression {
                crate::config::ResidualSuppression::Balanced => (2.25, 4.5),
                _ => (3.0, 6.0),
            };
            // Apply consistently to mono and detected stereo. A stationary
            // independent near-end signal may never trigger speech detection.
            for c in [&mut config, &mut stereo] {
                c.suppressor.normal_tuning = c.suppressor.nearend_tuning.clone();
                for tuning in [&mut c.suppressor.normal_tuning, &mut c.suppressor.nearend_tuning] {
                    tuning.mask_lf.enr_transparent = transparent;
                    tuning.mask_lf.enr_suppress = suppress;
                    tuning.mask_hf = tuning.mask_lf.clone();
                }
            }
        }
        let multichannel_config = Some(stereo);
